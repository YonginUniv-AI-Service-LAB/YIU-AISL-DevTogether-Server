...
public class MatchingController {
    ...
    public ResponseEntity<Boolean> applyMentor(@AuthenticationPrincipal CustomUserDetails customUserDetails, MatchingRequestDto.MentorApplyDTO request) throws Exception {
        return new ResponseEntity<Boolean>(matchingService.applyMentor(customUserDetails, request), HttpStatus.OK);
    }

    // 신청하기(멘토가 멘티에게)
    @PostMapping("/matching/application/mentee")
    public ResponseEntity<Boolean> applyMentee(@AuthenticationPrincipal CustomUserDetails customUserDetails, MatchingRequestDto.MenteeApplyDTO request) throws Exception {
        return new ResponseEntity<Boolean>(matchingService.applyMentee(customUserDetails, request), HttpStatus.OK);
    }
